...
?>
	...
	if ($protect) {
		$viewer_base_url = PDFPRO_PLUGIN_DIR . "pdfjs-new/web/pviewer.php";
	} else {
	...
	$options = [
		...
		'disableContent' => (bool)get_post_meta($id, 'pdfp_onei_pp_right_click', true)
	]
	?>
	<?php if (!empty($file_url)) { ?>
		<div class="pdfp_main_wrapper" data-option='<?php echo wp_json_encode($options) ?>' style="width:100%; overflow:hidden;">
			<div class="pdfp_cta_wrapper" style="width:100%; overflow:hidden; float:left;">
				<?php if ($show_filename == '1' && !$protect): ?>File name : <?php $name = basename($file_url);
																				echo $name; ?><br /> <?php endif; ?>
